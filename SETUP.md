...
cron-job.org (зовнішній щоденний тригер), Google Sheets/Drive (Service
...
## Крок 8. Автоматичний щоденний запуск — через cron-job.org
...
3. У cron-job.org створи новий Cron Job:
   ...
   - **Розклад:** щодня, 09:00, часовий пояс `Europe/Kyiv` (сервіс сам
     враховує перехід на літній/зимовий час — окремих двох тригерів,
     як раніше в GitHub cron, не потрібно).
4. Перевір тестовим запуском кнопки "Run now" у cron-job.org — очікувана
   ...
   запуск workflow "Daily Job Search". Після цього перевір **Execution
   history** у cron-job.org за наступну добу — там має бути рівно ОДИН
   запис на день.

## Крок 9. Перевір вручну (без очікування щоденного тригера)
